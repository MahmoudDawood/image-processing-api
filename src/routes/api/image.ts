...
route.get('/', async(req, res) => {
  ...
  if(filename === "") return res.status(400).send('filename parameter value is required')
  if(width === "") return res.status(400).send('width parameter value is required')
  if(height === "") return res.status(400).send('height parameter value is required')
